...
local rawUrl = "https://raw.githubusercontent.com/xandercreates/JustLean-3/refs/heads/main/just-lean-3-lite.lua"
...
---@return table[]
function jl3.lean:new(mode, part, speed, pivot, enabled, constraints, strength, dobreathe, dospring, doshimmy, damp_shimmy)
...
function lean:tick()
    ...
    if self.do_spring then
        ...
        self.rot = lerp(self.rot, self.r_t, self.speed)
        self.pivot = lerp(self.pivot, self.pvt_t, self.speed)
        self.shimmy = lerp(self.shimmy, self.sway_t, self.speed)
    end
end
...
function head:tick()
    ...
    self.rot = lerp(self.rot, calc, self.speed) * self._w
end
...
function legs:tick()
    ...
    self.rot = lerp(self.rot, r, self.speed) * self._w
    self.pos = lerp(self.pos, p, self.speed) * self._w
end
